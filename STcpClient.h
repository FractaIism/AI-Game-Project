...
/*
	...
	listStep = [(r1, c1), (r2, c2) ...]
	r1, c1 表示要移動的棋子座標 (row, column) (zero-base)
	ri, ci (i>1) 表示該棋子移動路徑
*/
void SendStep(int id_package, std::vector<std::vector<int>>& listStep) {
	if (socketServer == INVALID_SOCKET) {
		printf("[Error] : trying to send step before connection is established\n");
		return;
	}

	int cntStep = listStep.size();
	if (cntStep > 100) {
		printf("[Error] : step too long(must less than 100)\n");
		cntStep = 100;
	}
	std::vector<BYTE> rbData;
	rbData.resize(8 + cntStep * 8);
	int* rbInsert = (int*)&(rbData[0]);
	*rbInsert++ = cntStep;
	*rbInsert++ = id_package;
	for (int i = 0; i < cntStep; ++i) {
		*rbInsert++ = listStep[i][0];
		*rbInsert++ = listStep[i][1];
	}
